...
def make_chart(path):
    ...
    with open(path, 'r') as f:
        ...
        for idx, row in enumerate(data[1:]):
            def safe_parse(s):
                try:
                    # assert s[:-2] == 'AM' or s[:-2] == 'PM'
                    return parser.parse(s)
                except Exception as e:
                    raise ValueError(f"Error parsing \"{s}\" in {path} at row {idx}, with message: {e}")
            ...
            if delta > datetime.timedelta(hours=5):
                print(f"Warning: {delta} is a long time for {category} in {path} at row {idx}. Start: \"{start_s}\", End: \"{end_s}\"")
